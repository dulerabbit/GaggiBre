    ...
    if (!recording && !extendedRecording && isFileOpen) {
        ...
        if (duration <= MIN_RETAINED_SHOT_MS) { // Exclude near-instant aborts only
            ...
            if (!appendToIndex(indexEntry)) {
                ESP_LOGW("ShotHistoryPlugin", "Index append failed for shot %u – retrying on SPIFFS fallback", indexEntry.id);
                if (fs != &SPIFFS) {
                    fs = &SPIFFS;
                    if (!fs->exists("/h")) {
                        fs->mkdir("/h");
                    ...
                    ensureIndexExists();
                    if (!appendToIndex(indexEntry)) {
                        ESP_LOGE("ShotHistoryPlugin", "CRITICAL: Failed to add shot %u to index even on SPIFFS", indexEntry.id);
                    }
                } else {
